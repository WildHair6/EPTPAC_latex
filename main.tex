...
Case~1 verifies fundamental performance under dual saturation.
...
As precision increases, however, the required gains grow substantially: tightening $\varepsilon_1$ from $10^{-4}$ to $10^{-7}$
...
Case~2 confirms that the EPTPAC framework delivers exact terminal-time convergence
with high temporal precision and tunable steady-state accuracy.

\subsection{Case 3: Comparison with Ref.~\cite{xu_distributed_2022}}

To evaluate the advantage of the proposed framework over existing methods, this case compares EPTPAC with Ref.~\cite{xu_distributed_2022}, which addresses torque limits but does not account for wheel-momentum constraints.
...
This scenario compares terminal-time tracking performance under torque-only saturation
...
Ref.~\cite{xu_distributed_2022} exhibits pronounced conservatism, converging around $170~\si{s}$
despite the prescribed $T_f=300~\si{s}$.
In contrast, EPTPAC achieves exact convergence at $t=300~\si{s}$ without saturating the control torque.

\subsubsection{Case 3.2: Dual saturation}

This scenario evaluates controller performance under dual saturation
($\tau_{i,\max}=0.2~\si{N\cdot m}$, $h_{w,i,\max}=4.0~\si{kg\cdot m^2/s}$) with $T_f=300~\si{s}$.
Under the momentum constraint,
Ref.~\cite{xu_distributed_2022} loses prescribed-time convergence, with convergence delayed to approximately $410~\si{s}$.
...
neglects momentum constraints and commands impulsive torques, rapidly driving the wheels into saturation.
Once saturated, the spacecraft loses control authority and enters an uncontrolled drift phase.
In contrast, EPTPAC modulates the torque profile to prevent momentum saturation,
achieving exact convergence at $t=300~\si{s}$.

In summary, Case~3 demonstrates that the EPTPAC framework eliminates the conservatism inherent in existing PTC methods and accommodates dual-saturation constraints.
