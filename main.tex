...
making the notion of prescribed time meaningless under practical constraints.
...
$\lambda_{\min}(\boldsymbol{A})$ and $\lambda_{\max}(\boldsymbol{A})$ are the extreme eigenvalues of $\boldsymbol{A}$.
...
\end{equation}
where $\boldsymbol{\sigma}\in\mathbb{R}^3$ represents the attitude in Modified Rodrigues Parameters (MRPs) and $\boldsymbol{\omega}\in\mathbb{R}^3$ the body angular velocity,
...
The origin of \eqref{eq:system} is said to be prescribed-time stable if, with $T_p>0$ and any initial condition, $\boldsymbol{x}(t)=\boldsymbol{0}$ for all $t\ge T_p$ \cite{sanchez-torres_predefined-time_2015}, and prescribed-time prescribed-accuracy stable if, with $\varepsilon>0$, also $\|\boldsymbol{x}(t)\|\le\varepsilon$ for all $t\ge T_p$ \cite{xiao_scaling-transformation-based_2025}.
...
The reference momentum $\boldsymbol{H}_d = \boldsymbol{J}_0\boldsymbol{\omega}_d + \boldsymbol{h}_{w,d}$ is generated under the disturbance-free dynamics of the OCP, where the magnitude of the total angular momentum is conserved. Hence, with $\boldsymbol{H}_d(0)=\boldsymbol{H}(0)=\boldsymbol{H}_0$ and $H_0 \triangleq \|\boldsymbol{H}_0\|$, we have
\begin{equation}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[height=0.35\textheight]{fig/overall.pdf}
