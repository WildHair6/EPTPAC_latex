...
Once momentum saturation occurs, the spacecraft risks losing control authority.

Conventional finite-time and fixed-time control methods
...
for instance, in Ref.~\cite{xu_distributed_2024}, convergence occurs at $250~\mathrm{s}$ for $T_f=500~\mathrm{s}$.
This premature settling forces an idle phase and the aggressive initial effort
accelerates momentum accumulation.

Beyond conservatism, the physical realizability of PTC under dual saturation
has received little attention and entails two consequences.
...
angular-momentum saturation is seldom treated as a coupled constraint;
reaching the momentum limit causes complete loss of control authority
regardless of whether torque commands remain within bounds.
...
Prior constrained attitude control methods
\cite{wallsgrove_globally_2005,hu_finite-time_2017,miller_guidance_2024}
provide torque-saturation guarantees but do not address angular-momentum limits.
Optimization-based planning
\cite{kobayashi_optimal_2017,spiller_inverse_2018,celani_spacecraft_2020,fan_polynomial_2024}
is inherently open-loop, and model predictive control
\cite{mammarella_attitude_2019,yang_attitude_2023}
ensures only asymptotic convergence without angular-momentum constraints.
No existing approach simultaneously achieves exact terminal-time convergence,
prescribed accuracy, and dual-saturation compliance.
A systematic solution remains to be developed.

Motivated by the above analysis, this note proposes an exact prescribed-time
prescribed-accuracy control (EPTPAC) framework with a two-loop architecture.
The outer loop generates a dual-saturation-feasible reference trajectory reaching the target
precisely at $T_f$ via constrained optimal control.
...
A parameter-synthesis methodology coordinates the two loops,
ensuring closed-loop feasibility under dual saturation.
...
$\boldsymbol{h}_w\in\mathbb{R}^3$ and $\boldsymbol{\tau}\in\mathbb{R}^3$ are the wheel angular momentum and control torque,
...
Let $\boldsymbol{H}=\boldsymbol{J}\boldsymbol{\omega}+\boldsymbol{h}_w$ be the total angular momentum and $\boldsymbol{H}_0=\boldsymbol{H}(0)$.
...
The origin is prescribed-time stable if $\boldsymbol{x}(t)=\boldsymbol{0}$ for all $t\ge T_p$
\cite{sanchez-torres_predefined-time_2015},
and prescribed-time prescribed-accuracy stable if $\|\boldsymbol{x}(t)\|\le\varepsilon$ for all $t\ge T_p$
...
Introducing the additional angular-momentum constraint ($h_{w,i,\max}=4.0~\si{kg\cdot m^2/s}$) with $T_f=300~\si{s}$
...
neglects angular-momentum constraints and commands impulsive torques, rapidly saturating the wheels.
...
The reference angular momentum $\boldsymbol{H}_d = \boldsymbol{J}_0\boldsymbol{\omega}_d + \boldsymbol{h}_{w,d}$,
