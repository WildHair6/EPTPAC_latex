...
The OCP is solved via GPOPS-II \cite{patterson_gpops-ii_2014}.
The resulting discrete nodal solution is reconstructed as a continuous-time reference via piecewise-linear interpolation.
The reference trajectory has bounded angular velocity and angular acceleration; specifically, there exist constants $\omega_{d,\max}>0$ and $\dot{\omega}_{d,\max}>0$ such that
$\|\boldsymbol{\omega}_d(t)\| \le \omega_{d,\max}$ and
$\|\dot{\boldsymbol{\omega}}_d(t)\| \le \dot{\omega}_{d,\max}$ for all $t\in[0,T_f]$.

The outer loop thus provides a reference trajectory that reaches the target state exactly at $T_f$, serving as the nominal reference for the inner-loop tracking controller.
...
\end{equation}
where $\varepsilon_1 > 0$, $T_{p1} > 0$, and $\alpha_1 > 0$ are design parameters, and $\eta$ is as in Lemma~\ref{lem:PT}. The gains are given by
\[
...
To drive the sliding variable $\boldsymbol{s}$ to zero, the inner-loop control law is designed as
\begin{equation}
...
By appropriately selecting $\alpha_1,\alpha_2>0$, one can guarantee that
