...
\begin{figure}[ht]
    \centering
    \includegraphics[height=0.45\textheight]{fig/overall.pdf}
...
Three cases are conducted: Case~1 validates nominal tracking performance and dual-saturation compliance, Case~2 assesses exact prescribed-time convergence and accuracy tunability, and Case~3 benchmarks the proposed framework against the torque-saturated prescribed-time controller in~\cite{xu_distributed_2022} (hereafter Ref.~\cite{xu_distributed_2022}) under torque-only and dual-saturation conditions.
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.68\linewidth]{fig/Case1_Results_ErrorNorms_tf120.0.pdf}
...
Fig.~\ref{fig:Case2_qe_xyz} shows that the attitude tracking error converges at the respective prescribed terminal times for all tested values, 
entering the prescribed accuracy only $0.27$--$0.37~\si{s}$ prior to $T_f$.
...
As indicated in Fig.~\ref{fig:case4_1_tf300}, for $T_f=300~\si{s}$, Ref.~\cite{xu_distributed_2022} converges around $150~\si{s}$, exhibiting pronounced conservatism at only 50\% of the prescribed maneuver duration.
...
However, when $T_f$ is reduced to $150~\si{s}$, the prescribed-time property breaks down.
As illustrated in Fig.~\ref{fig:case4_1_tf150}, the response does not converge at $150~\si{s}$ but is instead delayed to approximately $230~\si{s}$.
