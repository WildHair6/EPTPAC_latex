...
to within a prescribed accuracy before $T_f$, despite inertia uncertainties and disturbances.
...
Specify $T_f$, $\varepsilon_{\mathrm{mission}}$, $\tau_{\max}$, $h_{w,\max}$, $T_{d,\max}$, and $H_{\max}$.
...
\begin{figure}[htbp]
    \centering
    \includegraphics[height=0.3\textheight]{fig/overall.pdf}
...
As shown in Fig.~\ref{fig:convergence_behavior}, all tracking errors remain within the prescribed-accuracy sets over $[0,T_f]$, consistent with the computed convergence bounds $\varepsilon_{\delta_1},\varepsilon_{\delta_2}$.
...
At $T_f=300~\si{s}$, Ref.~\cite{xu_distributed_2022} converges around $150~\si{s}$ (Figs.~\ref{fig:err300} and~\ref{fig:tau300}), exhibiting pronounced conservatism, converging at only 50\% of the prescribed duration.
When $T_f$ is reduced to $150~\si{s}$, approaching $T_{f,\min}$, Ref.~\cite{xu_distributed_2022} loses prescribed-time convergence entirely, with the response delayed to approximately $230~\si{s}$ (Figs.~\ref{fig:err150} and~\ref{fig:tau150}).
The high-gain feedback mechanism does not account for physical feasibility near $T_{f,\min}$, and the resulting torque saturation compromises the convergence guarantees.
...
In summary, Case~3 demonstrates that neglecting $T_{f,\min}$ may lead to loss of prescribed-time convergence, and that ignoring momentum saturation may result in loss of control authority. The proposed EPTPAC framework eliminates the conservatism inherent in existing PTC methods while rigorously accounting for physical feasibility.

\section{Conclusion}
\label{section:conclusion}
This note proposed a two-loop EPTPAC framework for spacecraft actuated by reaction wheels to achieve exact convergence at a prescribed terminal time with prescribed accuracy under dual saturation.
Through this decoupled architecture, the proposed framework effectively overcomes the fundamental shortcomings of existing PTC methods, namely the inherent conservatism and insufficient consideration of dual saturation.
Specifically, rigorous axis-by-axis feasibility conditions and an estimate of the minimum feasible maneuver time $T_{f,\min}$ are derived, ensuring dual-saturation compliance while guaranteeing that the prescribed terminal time is physically realizable.
Building on these conditions, a systematic parameter-synthesis methodology coordinates the two loops, requiring only three free design parameters.
Numerical simulations confirmed exact prescribed-time convergence with high temporal precision, prescribed-accuracy tunability across four orders of magnitude, and dual-saturation compliance.
