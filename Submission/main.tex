...
\author{Zaihua Liu\footnote{Ph.D. Candidate, Research Center of Satellite Technology, State Key Laboratory of Micro-Spacecraft Rapid Design and Intelligent Cluster.}, Yan Xiao\footnote{Associate Professor, Research Center of Satellite Technology, State Key Laboratory of Micro-Spacecraft Rapid Design and Intelligent Cluster. xiaoy@hit.edu.cn (Corresponding Author).}, and Dong Ye\footnote{Professor, Research Center of Satellite Technology, State Key Laboratory of Micro-Spacecraft Rapid Design and Intelligent Cluster.}}
\affil{Harbin Institute of Technology, Harbin 150001, People's Republic of China}
\author{Haiyue Yang\footnote{Ph.D. Candidate, Department of Signal Theory and Communications.}}
\affil{Universitat Polit\`{e}cnica de Catalunya, Barcelona 08034, Spain}
\author{and Zhaowei Sun\footnote{Professor, Research Center of Satellite Technology, State Key Laboratory of Micro-Spacecraft Rapid Design and Intelligent Cluster.}}
\affil{Harbin Institute of Technology, Harbin 150001, People's Republic of China}
...
While dual saturation constraint can be explicitly incorporated through
...
To address this problem, this paper proposes an exact prescribed-time
...
\begin{figure}[ht]
    \centering
    \includegraphics[height=0.4\textheight]{overall.pdf}
    \caption{Parameter-synthesis procedure.}
...
For all simulations, the initial attitude is set to $\boldsymbol{\sigma}(0)=[0.2,\ 0.3,\ -0.3]^\top$ and the target attitude is $\boldsymbol{\sigma}_{\mathrm{target}}=\boldsymbol{0}$. The spacecraft starts and ends at rest with $\boldsymbol{\omega}(0)=\boldsymbol{\omega}_{\mathrm{target}}=\boldsymbol{0}~\si{rad/s}$.
The common physical and disturbance parameters are listed in Table~\ref{tab:sim_setup_all}.

\begin{table}[ht]
\centering
\caption{Common simulation settings.}
...
$\SI{0.03}{N\cdot m}$ \\
$H_{\max}$ &
$\SI{0.1}{kg\cdot m^2/s}$ \\\bottomrule
\end{tabular}
\end{table}

In each case, the nominal reference trajectory $(\boldsymbol{\sigma}_d,\boldsymbol{\omega}_d)$ is generated offline by solving the constrained OCP \eqref{eq:OCP_slack} via GPOPS-II, and is held constant at the target state for $t\ge T_f$.
Following Remark~\ref{rem:gamma_selection}, $\gamma_u=0.2$ and $\gamma_h=0.1$ are adopted.
From Appendix~\ref{app:bangbang}, the minimum feasible terminal time is $T_{f,\min}\approx\SI{95}{s}$. Therefore, $T_f\ge\SI{110}{s}$ is employed in all cases to ensure feasibility.

To comprehensively evaluate the framework, three cases are conducted: Case~1 validates the nominal performance; Case~2 assesses exact prescribed-time convergence and prescribed-accuracy performance; and Case~3 benchmarks the proposed framework against the torque-saturated prescribed-time controller in~\cite{xu_distributed_2022} (hereafter Ref.~\cite{xu_distributed_2022}) under both torque-only and dual-saturation conditions.
...
Following the synthesis method in Section~\ref{overall_design} with $\kappa=1.2$, the inner-loop parameters are set as $\varepsilon_2=6.43\times10^{-5}$ and $\alpha_1=\alpha_2=6.34$.
...
Fig.~\ref{fig:tracking_performance} shows that the spacecraft attitude and angular velocity closely track the 
reference trajectory, which reaches the target state exactly at $t = T_f$.
Furthermore, Fig.~\ref{fig:convergence_behavior} shows that all tracking errors remain within the prescribed-accuracy sets over $[0,T_f]$, while Fig.~\ref{fig:constraint_satisfaction} confirms that both the control torque and wheel momentum remain within their respective saturation limits throughout the maneuver.

\begin{figure}[ht]
    \centering
    \includegraphics[width=0.55\linewidth]{Case1_Results_ErrorNorms_tf120.0.pdf}
    \caption{Case~1: Log-scale tracking-error convergence.}
...
In summary, Case~1 confirms the exact prescribed-time convergence, prescribed-accuracy performance, and dual-saturation compliance of the proposed EPTPAC framework.

\subsection{Case 2: Prescribed Performance Assessment}
\label{sec:case2_prescribed_perf}

This case evaluates the prescribed-time convergence and prescribed-accuracy performance of the EPTPAC framework.

\subsubsection{Prescribed-time convergence.}
Here, the target-pointing MRP error $\boldsymbol{\sigma}_{e,\mathrm{tar}}$, defined between $\boldsymbol{\sigma}(t)$ and $\boldsymbol{\sigma}_{\mathrm{target}}$, is used to distinguish it from the reference-tracking error $\boldsymbol{\sigma}_e$.
...
For each $T_f$, the inner-loop parameters are synthesized via the balanced rule with $\kappa=1.2$
following the procedure in Section~\ref{overall_design}.
The results are as follows.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.55\linewidth]{qe_norm_comparison.pdf}
  \caption{Case~2: Target-pointing MRP error norm for varying $T_f$.}
  \label{fig:Case2_qe_norm}
\end{figure}

Fig.~\ref{fig:Case2_qe_norm} shows that the target-pointing error norm $\|\boldsymbol{\sigma}_{e,\mathrm{tar}}\|$ converges at the respective prescribed terminal times for all tested values,
entering the prescribed accuracy only $0.24$--$0.27~\si{s}$ prior to $T_f$.
...
\begin{figure}[ht]
    ...
    \begin{minipage}[b]{0.45\linewidth}\centering
        ...
        \footnotesize (a) $\log_{10}(\|\boldsymbol{\sigma}_e\|)$
    \end{minipage}
    \hspace{0.05\linewidth}
    \begin{minipage}[b]{0.45\linewidth}\centering
        \includegraphics[width=\linewidth]{CaseSweep_SlidingNorm_log10.pdf}\\[2pt]
        \footnotesize (b) $\log_{10}(\|\boldsymbol{s}\|)$
    \end{minipage}
    \caption{Case~2: Log-scale error and sliding-variable norms for varying $\varepsilon_1$.}
    \label{fig:case2_precision_compare}
\end{figure}

Fig.~\ref{fig:case2_precision_compare} shows that all four settings achieve their prescribed accuracies within $T_f$.
However, as the prescribed accuracy tightens, the required gains grow substantially: reducing $\varepsilon_1$ from $10^{-4}$ to $10^{-7}$
...
In summary, Case~2 validates the exact prescribed-time convergence and prescribed-accuracy performance of the proposed EPTPAC framework through sweeps of $T_f$ and $\varepsilon_1$, respectively.
...
For fairness, all physical parameters, external disturbances, and initial conditions are kept identical to those in Table~\ref{tab:sim_setup_all}.
Two scenarios are considered: Case~3.1 evaluates
torque saturation only under identical constraint assumptions to provide a fair comparison,
while Case~3.2 enforces dual saturation to assess how the
...
\begin{figure}[ht]
    \centering
    \begin{minipage}[b]{0.43\linewidth}\centering
        \includegraphics[width=\linewidth]{PTC_compare_Attitude__t_total400.0_eptpac.pdf}\\[2pt]
        \footnotesize (a) Attitude error
    \end{minipage}
    \hspace{0.05\linewidth}
    \begin{minipage}[b]{0.43\linewidth}\centering
        \includegraphics[width=\linewidth]{PTC_compare_ControlTorque_t_total400.0_eptpac.pdf}\\[2pt]
        \footnotesize (b) Control torque
    \end{minipage}
    \caption{Case~3.1: Torque-only saturation with $T_f=\SI{300}{s}$.}
    \label{fig:case4_1_tf300}
\end{figure}

Fig.~\ref{fig:case4_1_tf300} shows that for $T_f=300~\si{s}$, Ref.~\cite{xu_distributed_2022} converges around $170~\si{s}$, exhibiting pronounced conservatism at only about 57\% of the prescribed maneuver duration.

\begin{figure}[ht]
    \centering
    \begin{minipage}[b]{0.43\linewidth}\centering
        \includegraphics[width=\linewidth]{PTC_compare_Attitude__t_total300.0_eptpac.pdf}\\[2pt]
        \footnotesize (a) Attitude error
    \end{minipage}
    \hspace{0.05\linewidth}
    \begin{minipage}[b]{0.43\linewidth}\centering
        \includegraphics[width=\linewidth]{PTC_compare_ControlTorque_t_total300.0_eptpac.pdf}\\[2pt]
        \footnotesize (b) Control torque
    \end{minipage}
    \caption{Case~3.1: Torque-only saturation with $T_f=\SI{150}{s}$.}
    \label{fig:case4_1_tf150}
\end{figure}

However, when $T_f$ is reduced to $150~\si{s}$, the prescribed-time convergence no longer holds.
Fig.~\ref{fig:case4_1_tf150} shows that the response does not converge at $150~\si{s}$ but is instead delayed to approximately $260~\si{s}$.
...
\begin{figure}[ht]
    \centering
    \begin{minipage}[b]{0.43\linewidth}\centering
        \includegraphics[width=\linewidth]{PTC_compare_Attitude__t_total500.0_eptpac.pdf}\\[2pt]
        \footnotesize (a) Attitude error
    \end{minipage}
    \hspace{0.04\linewidth}
    \begin{minipage}[b]{0.43\linewidth}\centering
        \includegraphics[width=\linewidth]{PTC_compare_omega__t_total500.0_eptpac.pdf}\\[2pt]
        \footnotesize (b) Angular velocity error
    \end{minipage}\\[1pt]
    \begin{minipage}[b]{0.43\linewidth}\centering
        \includegraphics[width=\linewidth]{PTC_compare_ControlTorque_t_total500.0_eptpac.pdf}\\[2pt]
        \footnotesize (c) Control torque
    \end{minipage}
    \hspace{0.04\linewidth}
    \begin{minipage}[b]{0.43\linewidth}\centering
        \includegraphics[width=\linewidth]{PTC_compare_WheelMomentum_t_total500.0_eptpac.pdf}\\[2pt]
        \footnotesize (d) Wheel momentum
    \end{minipage}
...
As shown in Fig.~\ref{fig:case4_2_comparison}, under dual saturation Ref.~\cite{xu_distributed_2022} fails to achieve prescribed-time convergence, with the settling time delayed to approximately $410~\si{s}$. This failure stems from the lack of momentum management: aggressive torque commands rapidly drive the wheels into saturation, leading to loss of control authority and uncontrolled drift.
